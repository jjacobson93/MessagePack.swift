...
class ConveniencePropertiesTests: XCTestCase {
    ...
    func testStringValueWithCompatibility() {
        let stringValue = MessagePackValue.Binary(Data(bytes: [0x48, 0x65, 0x6c, 0x6c, 0x6f, 0x2c, 0x20, 0x77, 0x6f, 0x72, 0x6c, 0x64, 0x21])).stringValue
    ...
    func testDataValue() {
        ...
        var dataValue = MessagePackValue.Binary(Data(bytes: [0x00, 0x01, 0x02, 0x03, 0x04])).dataValue
        XCTAssertNotNil(dataValue)
        XCTAssertEqual(dataValue!, Data(bytes: [0x00, 0x01, 0x02, 0x03, 0x04]))

        dataValue = MessagePackValue.Extended(4, Data(bytes: [0x00, 0x01, 0x02, 0x03, 0x04])).dataValue
        XCTAssertNotNil(dataValue)
        XCTAssertEqual(dataValue!, Data(bytes: [0x00, 0x01, 0x02, 0x03, 0x04]))
    }
    ...
    func testExtendedValue() {
        ...
        let extended = MessagePackValue.Extended(4, Data(bytes: [0x00, 0x01, 0x02, 0x03, 0x04]))
        let tuple = extended.extendedValue
        XCTAssertNotNil(tuple)

        let (type, data) = tuple!
        XCTAssertEqual(type, 4)
        XCTAssertEqual(data, Data(bytes: [0x00, 0x01, 0x02, 0x03, 0x04]))
    }
