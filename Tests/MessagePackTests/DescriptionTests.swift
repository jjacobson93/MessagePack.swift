...
class DescriptionTests: XCTestCase {
    ...
    func testBinaryDescription() {
        XCTAssertEqual(MessagePackValue.Binary(Data()).description, "Data([])")
        XCTAssertEqual(MessagePackValue.Binary(Data(bytes: [0x00, 0x01, 0x02, 0x03, 0x04])).description, "Data([0x00, 0x01, 0x02, 0x03, 0x04])")
    }
    ...
    func testMapDescription() {
        ...
        for indices in indexPermutations {
            let permutation = indices.map({ components[$0] })
    ...
    func testExtendedDescription() {
        XCTAssertEqual(MessagePackValue.Extended(5, Data()).description, "Extended(5, [])")
        XCTAssertEqual(MessagePackValue.Extended(5, Data(bytes: [0x00, 0x10, 0x20, 0x30, 0x40])).description, "Extended(5, [0x00, 0x10, 0x20, 0x30, 0x40])")
    }
}
