...
class ConvenienceInitializersTests: XCTestCase {
    static var allTests : [(String, (ConvenienceInitializersTests) -> () throws -> Void)] {
        return [
            ("testNil", testNil),
            ("testBool", testBool),
            ("testUInt", testUInt),
    ...
    func testBinary() {
        ...
        XCTAssertEqual(MessagePackValue(data), MessagePackValue.Binary(Data(bytes: [0x00, 0x01, 0x02, 0x03, 0x04])))
    }
}
