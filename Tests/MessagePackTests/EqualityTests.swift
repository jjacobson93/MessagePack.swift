...
class EqualityTests: XCTestCase {
    static var allTests : [(String, (EqualityTests) -> () throws -> Void)] {
        return [
    ...
    func testBinaryEqualToBinary() {
        XCTAssertEqual(MessagePackValue.Binary(Data(bytes: [0x00, 0x01, 0x02, 0x03, 0x04])), MessagePackValue.Binary(Data(bytes: [0x00, 0x01, 0x02, 0x03, 0x04])))
    }
    ...
    func testExtendedEqualToExtended() {
        XCTAssertEqual(MessagePackValue.Extended(5, Data(bytes: [0x00, 0x01, 0x02, 0x03, 0x04])), MessagePackValue.Extended(5, Data(bytes: [0x00, 0x01, 0x02, 0x03, 0x04])))
    }
}