...
class HashValueTests: XCTestCase {
    static var allTests : [(String, (HashValueTests) -> () throws -> Void)] {
        return [
    ...
    func testBinaryHashValue() {
        XCTAssertEqual(MessagePackValue.Binary(Data(bytes: [])).hashValue, 0)
        XCTAssertEqual(MessagePackValue.Binary(Data(bytes: [0x00, 0x01, 0x02, 0x03, 0x04])).hashValue, 5)
    }
    ...
    func testExtendedHashValue() {
        XCTAssertEqual(MessagePackValue.Extended(5, Data(bytes: [])).hashValue, Int(5).hashValue ^ Int(0))
        XCTAssertEqual(MessagePackValue.Extended(5, Data(bytes: [0x00, 0x01, 0x02, 0x03, 0x04])).hashValue, Int(5).hashValue ^ Int(5))
    }
}
